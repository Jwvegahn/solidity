...
.. code-block:: solidity
    ...
    // also a base class of `Pausable`, yet there is only a single
    // instance of `Owned` (as for virtual inheritance in C++).
    contract Named is Owned, Pausable {
    ...
    contract PriceFeed is Owned, Pausable, Named("GoldFeed") {
        uint info;
        ...
        // cannot change the behavior of `pause` anymore.
        function pause() public override(Pausable, Named) { Named.pause(); }
        function get() public view returns(uint r) { return info; }
    }

Note that above, we call ``Pausable.pause()`` to "forward" the
pause request. The way this is done is problematic, as
...
.. code-block:: solidity
    ...
    contract Base1 is Pausable {
        event Paused_Base1();
        function pause() public virtual override {
            /* Here, we emit an event to simulate some Base1 pause logic */
            emit Paused_Base1();
            Pausable.pause();
        }
    }

    contract Base2 is Pausable {
        event Paused_Base2();
        function pause() public virtual override {
            /* Here, we emit an event to simulate some Base2 pause logic */
            emit Paused_Base2();
            Pausable.pause();
        }
    }
    ...
    contract Final is Base1, Base2 {
        event Paused_Final();
        function pause() public override(Base1, Base2) {
            /* Here, we emit an event to simulate some Final pause logic */
            emit Paused_Final();
            Base2.pause();
        }
    }

A call to ``Final.pause()`` will call ``Base2.pause`` because we specify it
explicitly in the final override, but this function will bypass
``Base1.pause``. The way around this is to use ``super``:
...
derived contract: Final, Base2, Base1, Pausable, Owned).
